    ...
    UploadImage.prototype = {
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                try {
                    ...
                    if (json.state == 'SUCCESS') {
                        _this.imageList[$file.index()]=json; //多图上传图片顺序
        ...
        getInsertList: function () {
            ...
            for (i = 0; i < this.imageList.length; i++) {
            	if(this.imageList[i]==undefined){ //多图上传图片顺序 
            		continue;
            	}
        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    var newUrl = list[i].url.replace("D:\/开发项目\/yugyg\/WebRoot\/","");
                    img.setAttribute('src', urlPrefix + newUrl + (newUrl.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
                    img.setAttribute('_src', urlPrefix + newUrl);
